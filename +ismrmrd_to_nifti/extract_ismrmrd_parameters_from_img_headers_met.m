function h = extract_ismrmrd_parameters_from_img_headers_met(headtmp)
...
h.ImageOrientationPatient = [ -head.phase_dir(:)' -head.read_dir(:)'];
...
corner_first_slice(3)=head.position(3)  ...
...
corner_last_slice(3)=head.position(3) ...
    + (hdr.encoding.reconSpace.fieldOfView_mm.x / 2.0 ) * head.read_dir(3) ...
    + (hdr.encoding.reconSpace.fieldOfView_mm.y / 2.0 ) * head.phase_dir(3) ...
    - (hdr.encoding.reconSpace.fieldOfView_mm.z / 2.0 ) * head.slice_dir(3) ;
